...
import cljs.js_deps.jar_entry_names_STAR_;
import clojure.instant.indivisible_QMARK_;
...
public class Synchronization {
    ...
    private static void invokeAllCallables(Calculator c) {
        ...
        List<Callable<Result>> tasks = List.of(new MyCallable("One", c), new MyCallable("Two", c));
        try {
    ...
    private static class CalculatorSyncMethod implements Calculator {
        ...
        synchronized public double calculate(int i){
            try {
                ...
                Thread.currentThread().interrupt();
                System.out.println("Calculator was itnerrupted");
            }
            return 0.0;
        }
    }
    ...
    private static CalculatorNoSync implements Calculator {
        private double prop;
